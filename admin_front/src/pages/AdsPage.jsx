  ...
  return (
    <div className="fade-in">
      <div className="page-header">
        <div>
          <h1 className="page-title">{t('إدارة الإعلانات')}</h1>
          <p className="page-subtitle">{t('إدارة اللافتات الإعلانية المعروضة في واجهة المستخدمين')}</p>
        </div>
        <button className="btn btn-primary" onClick={() => setShowModal(true)}>
          <FiPlus size={14} style={{marginLeft:4}}/>{t('إضافة إعلان')}</button>
      </div>
      ...
      <div className="grid-3">
        {loading ? (
          ...
          <div style={{gridColumn:'1 / -1', textAlign:'center', padding:40, background:'var(--bg-card)', borderRadius:'var(--radius)'}}>{t('لا توجد إعلانات')}</div>
        ) : ads.map(ad => (
            ...
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', flex: 1 }}>
              ...
              <div style={{ marginTop: 'auto', display: 'flex', gap: 8 }}>
                <button className={`btn btn-sm ${ad.active ? 'btn-secondary' : 'btn-primary'}`} style={{ flex: 1, justifyContent: 'center' }}
                  ...
                  {ad.active ? <><FiToggleLeft size={14} style={{marginLeft:4}}/>{t('إيقاف')}</> : <><FiToggleRight size={14} style={{marginLeft:4}}/>{t('تفعيل')}</>}
                </button>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              <label className="form-label">{t('عنوان الإعلان')}</label>
              <input className="form-input" placeholder={t("مثال: عروض رمضان")} 
                value={form.title} onChange={e => setForm({...form, title: e.target.value})} />
            </div>

            <div className="form-group">
              <label className="form-label">الوصف (Description)</label>
              <textarea className="form-input" style={{ minHeight: 80, resize: 'vertical' }} placeholder={t("أدخل وصف الإعلان أو النص المصاحب له")}
                value={form.content} onChange={e => setForm({...form, content: e.target.value})} />
            </div>

            <div className="form-group">
              <label className="form-label">{t('صورة الإعلان')}</label>
              ...
              <div style={{ position:'relative' }}>
                ...
                <input className="form-input" style={{ paddingRight:36 }} placeholder={t("أو أدخل رابط الصورة (URL) هنا")} 
            ...
            <div className="grid-2">
              <div className="form-group">
                <label className="form-label">{t('الترتيب')}</label>
              ...
              <div className="form-group">
                ...
                <select className="form-select" value={form.active ? 'true' : 'false'} onChange={e => setForm({...form, active: e.target.value === 'true'})}>
                  ...
                  <option value="false">{t('متوقف')}</option>
                </select>
              </div>
            </div>
            ...
            {form.image_url && (
              <div style={{ marginBottom: 16 }}>
                <label className="form-label">{t('معاينة الصورة')}</label>
            ...
            <button className="btn btn-primary" style={{ width:'100%', justifyContent:'center' }} 
              onClick={handleSave} disabled={!form.title || (!form.image_url && !form.image_file)}>{t('حفظ الإعلان')}</button>
          </div>
        </div>
      )}
    </div>
