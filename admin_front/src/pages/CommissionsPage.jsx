  ...
  return (
    <div className="fade-in">
      <div className="page-header">
        <div>
          <h1 className="page-title">عروض العمولات</h1>
          <p className="page-subtitle">تكوين أسعار التكلفة وأسعار البيع ونسب الأرباح لكل خدمة ومستوى</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-secondary" onClick={load} disabled={loading}>
            <FiRefreshCw size={14} style={{marginLeft:4}}/> تحديث
          </button>
          <button className="btn btn-primary" onClick={openAdd}>
            <FiPlus size={14} style={{marginLeft:4}}/> إضافة عرض جديد
          </button>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          <span className="card-title">قائمة العروض المخصصة</span>
        </div>
        ...
        <div style={{ overflow: 'auto' }}>
          {loading ? (
            ...
            <div style={{ textAlign:'center', padding: 40, color: 'var(--text-muted)' }}>
              لا توجد عروض عمولات معرفة. قم بإضافة عرض جديد.
            </div>
          ) : (
            <table>
              <thead>
                <tr>
                  ...
                  <th>المبلغ الأساسي (Base)</th>
                  <th>سعر التكلفة (أنا)</th>
              ...
              <tbody>
                {offers.map(o => {
                  ...
                  return (
                    <tr key={o.id}>
                      <td style={{ fontWeight: 600 }}>{services.find(s => s.id === o.service)?.name || o.service}</td>
                      <td style={{ textTransform: 'capitalize' }}>{o.operator}</td>
                      <td>
                        <span className="badge-status" style={{ background: 'var(--bg-input)', color: 'var(--text-primary)' }}>
                          {roles.find(r => r.id === o.role)?.name || o.role}
                        </span>
                      </td>
                      <td style={{ fontWeight: 600 }}>{o.base_amount} د.ج</td>
                      <td style={{ color: 'var(--danger)' }}>{o.admin_cost} د.ج</td>
                      <td style={{ color: 'var(--primary)' }}>{o.client_price} د.ج</td>
                      <td>
                        <div style={{ color: 'var(--success)', fontWeight: 600 }}>{adminProfit} د.ج</div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{getPercentage(adminProfit, o.base_amount)}%</div>
                      </td>
                      <td>
                        <div style={{ color: 'var(--success)', fontWeight: 600 }}>{clientProfit} د.ج</div>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 500 }}>
            <div className="modal-header">
              <h3 className="modal-title">{editingId ? 'تعديل عرض عمولة' : 'إضافة عرض عمولة'}</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div className="form-group">
                <label className="form-label">المتعامل</label>
            ...
            <div className="form-group">
              <label className="form-label">مستوى المستخدم (Role)</label>
            ...
            <div className="form-group">
              <label className="form-label">المبلغ الأساسي كمثال (Base Amount)</label>
              <input className="form-input" type="number" value={form.base_amount} onChange={e => setForm({...form, base_amount: Number(e.target.value)})} />
              <small style={{ color: 'var(--text-muted)' }}>مثال: 10000 د.ج</small>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div className="form-group">
                <label className="form-label">سعر التكلفة (سعر شرائك)</label>
                <input className="form-input" type="number" value={form.admin_cost} onChange={e => setForm({...form, admin_cost: Number(e.target.value)})} />
                <small style={{ color: 'var(--danger)' }}>أنت تدفع: {form.admin_cost}</small>
              </div>
              <div className="form-group">
                <label className="form-label">سعر البيع (للزبون)</label>
                ...
                <small style={{ color: 'var(--primary)' }}>الزبون يدفع: {form.client_price}</small>
              </div>
            </div>

            <div style={{ padding: 12, background: 'var(--bg-sidebar)', borderRadius: 8, marginTop: 8, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span>ربحي أنا (Admin):</span>
                <strong style={{ color: 'var(--success)' }}>{form.client_price - form.admin_cost} د.ج ({getPercentage(form.client_price - form.admin_cost, form.base_amount)}%)</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>ربح الزبون:</span>
                <strong style={{ color: 'var(--success)' }}>{form.base_amount - form.client_price} د.ج ({getPercentage(form.base_amount - form.client_price, form.base_amount)}%)</strong>
              </div>
            </div>

            <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }} onClick={handleSave}>
              <FiSave size={16} style={{ marginLeft: 6 }} /> حفظ العرض
            </button>
          </div>
        </div>
      )}
